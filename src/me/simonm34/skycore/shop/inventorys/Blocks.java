...
public class Blocks {
    public Inventory build() {
        ...
        InventoryBuilder invBuilder = new InventoryBuilder().setSize(36).setName("&6&lSkyTreasures &8» &eBlocks Shop");
        for (String string : data.getConfigurationSection("shops.blocks.items").getKeys(false)) {
            Material material = Core.getCore().getUtils().parseMaterial(string);
            Integer quantity = data.getInt("shops.blocks.items." + string + ".quantity");
            Double buyPrice = data.getDouble("shops.blocks.items." + string + ".buy price");
            Double sellPrice = data.getDouble("shops.blocks.items." + string + ".sell price");
            Integer materialData = data.getInt("shops.blocks.items." + string + ".data");
