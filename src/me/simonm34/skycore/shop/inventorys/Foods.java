...
public class Foods {
    public Inventory build() {
        ...
        InventoryBuilder invBuilder = new InventoryBuilder().setSize(36).setName("&6&lSkyTreasures &8» &eFoods Shop");
        for (String string : data.getConfigurationSection("shops.foods.items").getKeys(false)) {
            Material material = Core.getCore().getUtils().parseMaterial(string);
            Integer quantity = data.getInt("shops.foods.items." + string + ".quantity");
            Double buyPrice = data.getDouble("shops.foods.items." + string + ".buy price");
            Double sellPrice = data.getDouble("shops.foods.items." + string + ".sell price");
            Integer materialData = data.getInt("shops.foods.items." + string + ".data");
