...
public class Drops {
    public Inventory build() {
        ...
        InventoryBuilder invBuilder = new InventoryBuilder().setSize(36).setName("&6&lSkyTreasures &8» &eDrops Shop");
        for (String string : data.getConfigurationSection("shops.drops.items").getKeys(false)) {
            Material material = Core.getCore().getUtils().parseMaterial(string);
            Integer quantity = data.getInt("shops.drops.items." + string + ".quantity");
            Double buyPrice = data.getDouble("shops.drops.items." + string + ".buy price");
            Double sellPrice = data.getDouble("shops.drops.items." + string + ".sell price");
            Integer materialData = data.getInt("shops.drops.items." + string + ".data");
